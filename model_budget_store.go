/*
Firefly III API v2.0.4

This is the documentation of the Firefly III API. You can find accompanying documentation on the website of Firefly III itself (see below). Please report any bugs or issues. You may use the \"Authorize\" button to try the API below. This file was last generated on 2023-06-11T09:14:35+00:00 

API version: 2.0.4
Contact: james@firefly-iii.org
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the BudgetStore type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BudgetStore{}

// BudgetStore struct for BudgetStore
type BudgetStore struct {
	Name string `json:"name"`
	Active *bool `json:"active,omitempty"`
	Order *int32 `json:"order,omitempty"`
	Notes NullableString `json:"notes,omitempty"`
	AutoBudgetType NullableAutoBudgetType `json:"auto_budget_type,omitempty"`
	// Use either currency_id or currency_code. Defaults to the user's default currency.
	AutoBudgetCurrencyId NullableString `json:"auto_budget_currency_id,omitempty"`
	// Use either currency_id or currency_code. Defaults to the user's default currency.
	AutoBudgetCurrencyCode NullableString `json:"auto_budget_currency_code,omitempty"`
	AutoBudgetAmount NullableString `json:"auto_budget_amount,omitempty"`
	AutoBudgetPeriod NullableAutoBudgetPeriod `json:"auto_budget_period,omitempty"`
}

// NewBudgetStore instantiates a new BudgetStore object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBudgetStore(name string) *BudgetStore {
	this := BudgetStore{}
	this.Name = name
	return &this
}

// NewBudgetStoreWithDefaults instantiates a new BudgetStore object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBudgetStoreWithDefaults() *BudgetStore {
	this := BudgetStore{}
	return &this
}

// GetName returns the Name field value
func (o *BudgetStore) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *BudgetStore) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *BudgetStore) SetName(v string) {
	o.Name = v
}

// GetActive returns the Active field value if set, zero value otherwise.
func (o *BudgetStore) GetActive() bool {
	if o == nil || IsNil(o.Active) {
		var ret bool
		return ret
	}
	return *o.Active
}

// GetActiveOk returns a tuple with the Active field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BudgetStore) GetActiveOk() (*bool, bool) {
	if o == nil || IsNil(o.Active) {
		return nil, false
	}
	return o.Active, true
}

// HasActive returns a boolean if a field has been set.
func (o *BudgetStore) HasActive() bool {
	if o != nil && !IsNil(o.Active) {
		return true
	}

	return false
}

// SetActive gets a reference to the given bool and assigns it to the Active field.
func (o *BudgetStore) SetActive(v bool) {
	o.Active = &v
}

// GetOrder returns the Order field value if set, zero value otherwise.
func (o *BudgetStore) GetOrder() int32 {
	if o == nil || IsNil(o.Order) {
		var ret int32
		return ret
	}
	return *o.Order
}

// GetOrderOk returns a tuple with the Order field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BudgetStore) GetOrderOk() (*int32, bool) {
	if o == nil || IsNil(o.Order) {
		return nil, false
	}
	return o.Order, true
}

// HasOrder returns a boolean if a field has been set.
func (o *BudgetStore) HasOrder() bool {
	if o != nil && !IsNil(o.Order) {
		return true
	}

	return false
}

// SetOrder gets a reference to the given int32 and assigns it to the Order field.
func (o *BudgetStore) SetOrder(v int32) {
	o.Order = &v
}

// GetNotes returns the Notes field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetNotes() string {
	if o == nil || IsNil(o.Notes.Get()) {
		var ret string
		return ret
	}
	return *o.Notes.Get()
}

// GetNotesOk returns a tuple with the Notes field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetNotesOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Notes.Get(), o.Notes.IsSet()
}

// HasNotes returns a boolean if a field has been set.
func (o *BudgetStore) HasNotes() bool {
	if o != nil && o.Notes.IsSet() {
		return true
	}

	return false
}

// SetNotes gets a reference to the given NullableString and assigns it to the Notes field.
func (o *BudgetStore) SetNotes(v string) {
	o.Notes.Set(&v)
}
// SetNotesNil sets the value for Notes to be an explicit nil
func (o *BudgetStore) SetNotesNil() {
	o.Notes.Set(nil)
}

// UnsetNotes ensures that no value is present for Notes, not even an explicit nil
func (o *BudgetStore) UnsetNotes() {
	o.Notes.Unset()
}

// GetAutoBudgetType returns the AutoBudgetType field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetAutoBudgetType() AutoBudgetType {
	if o == nil || IsNil(o.AutoBudgetType.Get()) {
		var ret AutoBudgetType
		return ret
	}
	return *o.AutoBudgetType.Get()
}

// GetAutoBudgetTypeOk returns a tuple with the AutoBudgetType field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetAutoBudgetTypeOk() (*AutoBudgetType, bool) {
	if o == nil {
		return nil, false
	}
	return o.AutoBudgetType.Get(), o.AutoBudgetType.IsSet()
}

// HasAutoBudgetType returns a boolean if a field has been set.
func (o *BudgetStore) HasAutoBudgetType() bool {
	if o != nil && o.AutoBudgetType.IsSet() {
		return true
	}

	return false
}

// SetAutoBudgetType gets a reference to the given NullableAutoBudgetType and assigns it to the AutoBudgetType field.
func (o *BudgetStore) SetAutoBudgetType(v AutoBudgetType) {
	o.AutoBudgetType.Set(&v)
}
// SetAutoBudgetTypeNil sets the value for AutoBudgetType to be an explicit nil
func (o *BudgetStore) SetAutoBudgetTypeNil() {
	o.AutoBudgetType.Set(nil)
}

// UnsetAutoBudgetType ensures that no value is present for AutoBudgetType, not even an explicit nil
func (o *BudgetStore) UnsetAutoBudgetType() {
	o.AutoBudgetType.Unset()
}

// GetAutoBudgetCurrencyId returns the AutoBudgetCurrencyId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetAutoBudgetCurrencyId() string {
	if o == nil || IsNil(o.AutoBudgetCurrencyId.Get()) {
		var ret string
		return ret
	}
	return *o.AutoBudgetCurrencyId.Get()
}

// GetAutoBudgetCurrencyIdOk returns a tuple with the AutoBudgetCurrencyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetAutoBudgetCurrencyIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AutoBudgetCurrencyId.Get(), o.AutoBudgetCurrencyId.IsSet()
}

// HasAutoBudgetCurrencyId returns a boolean if a field has been set.
func (o *BudgetStore) HasAutoBudgetCurrencyId() bool {
	if o != nil && o.AutoBudgetCurrencyId.IsSet() {
		return true
	}

	return false
}

// SetAutoBudgetCurrencyId gets a reference to the given NullableString and assigns it to the AutoBudgetCurrencyId field.
func (o *BudgetStore) SetAutoBudgetCurrencyId(v string) {
	o.AutoBudgetCurrencyId.Set(&v)
}
// SetAutoBudgetCurrencyIdNil sets the value for AutoBudgetCurrencyId to be an explicit nil
func (o *BudgetStore) SetAutoBudgetCurrencyIdNil() {
	o.AutoBudgetCurrencyId.Set(nil)
}

// UnsetAutoBudgetCurrencyId ensures that no value is present for AutoBudgetCurrencyId, not even an explicit nil
func (o *BudgetStore) UnsetAutoBudgetCurrencyId() {
	o.AutoBudgetCurrencyId.Unset()
}

// GetAutoBudgetCurrencyCode returns the AutoBudgetCurrencyCode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetAutoBudgetCurrencyCode() string {
	if o == nil || IsNil(o.AutoBudgetCurrencyCode.Get()) {
		var ret string
		return ret
	}
	return *o.AutoBudgetCurrencyCode.Get()
}

// GetAutoBudgetCurrencyCodeOk returns a tuple with the AutoBudgetCurrencyCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetAutoBudgetCurrencyCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AutoBudgetCurrencyCode.Get(), o.AutoBudgetCurrencyCode.IsSet()
}

// HasAutoBudgetCurrencyCode returns a boolean if a field has been set.
func (o *BudgetStore) HasAutoBudgetCurrencyCode() bool {
	if o != nil && o.AutoBudgetCurrencyCode.IsSet() {
		return true
	}

	return false
}

// SetAutoBudgetCurrencyCode gets a reference to the given NullableString and assigns it to the AutoBudgetCurrencyCode field.
func (o *BudgetStore) SetAutoBudgetCurrencyCode(v string) {
	o.AutoBudgetCurrencyCode.Set(&v)
}
// SetAutoBudgetCurrencyCodeNil sets the value for AutoBudgetCurrencyCode to be an explicit nil
func (o *BudgetStore) SetAutoBudgetCurrencyCodeNil() {
	o.AutoBudgetCurrencyCode.Set(nil)
}

// UnsetAutoBudgetCurrencyCode ensures that no value is present for AutoBudgetCurrencyCode, not even an explicit nil
func (o *BudgetStore) UnsetAutoBudgetCurrencyCode() {
	o.AutoBudgetCurrencyCode.Unset()
}

// GetAutoBudgetAmount returns the AutoBudgetAmount field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetAutoBudgetAmount() string {
	if o == nil || IsNil(o.AutoBudgetAmount.Get()) {
		var ret string
		return ret
	}
	return *o.AutoBudgetAmount.Get()
}

// GetAutoBudgetAmountOk returns a tuple with the AutoBudgetAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetAutoBudgetAmountOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AutoBudgetAmount.Get(), o.AutoBudgetAmount.IsSet()
}

// HasAutoBudgetAmount returns a boolean if a field has been set.
func (o *BudgetStore) HasAutoBudgetAmount() bool {
	if o != nil && o.AutoBudgetAmount.IsSet() {
		return true
	}

	return false
}

// SetAutoBudgetAmount gets a reference to the given NullableString and assigns it to the AutoBudgetAmount field.
func (o *BudgetStore) SetAutoBudgetAmount(v string) {
	o.AutoBudgetAmount.Set(&v)
}
// SetAutoBudgetAmountNil sets the value for AutoBudgetAmount to be an explicit nil
func (o *BudgetStore) SetAutoBudgetAmountNil() {
	o.AutoBudgetAmount.Set(nil)
}

// UnsetAutoBudgetAmount ensures that no value is present for AutoBudgetAmount, not even an explicit nil
func (o *BudgetStore) UnsetAutoBudgetAmount() {
	o.AutoBudgetAmount.Unset()
}

// GetAutoBudgetPeriod returns the AutoBudgetPeriod field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BudgetStore) GetAutoBudgetPeriod() AutoBudgetPeriod {
	if o == nil || IsNil(o.AutoBudgetPeriod.Get()) {
		var ret AutoBudgetPeriod
		return ret
	}
	return *o.AutoBudgetPeriod.Get()
}

// GetAutoBudgetPeriodOk returns a tuple with the AutoBudgetPeriod field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BudgetStore) GetAutoBudgetPeriodOk() (*AutoBudgetPeriod, bool) {
	if o == nil {
		return nil, false
	}
	return o.AutoBudgetPeriod.Get(), o.AutoBudgetPeriod.IsSet()
}

// HasAutoBudgetPeriod returns a boolean if a field has been set.
func (o *BudgetStore) HasAutoBudgetPeriod() bool {
	if o != nil && o.AutoBudgetPeriod.IsSet() {
		return true
	}

	return false
}

// SetAutoBudgetPeriod gets a reference to the given NullableAutoBudgetPeriod and assigns it to the AutoBudgetPeriod field.
func (o *BudgetStore) SetAutoBudgetPeriod(v AutoBudgetPeriod) {
	o.AutoBudgetPeriod.Set(&v)
}
// SetAutoBudgetPeriodNil sets the value for AutoBudgetPeriod to be an explicit nil
func (o *BudgetStore) SetAutoBudgetPeriodNil() {
	o.AutoBudgetPeriod.Set(nil)
}

// UnsetAutoBudgetPeriod ensures that no value is present for AutoBudgetPeriod, not even an explicit nil
func (o *BudgetStore) UnsetAutoBudgetPeriod() {
	o.AutoBudgetPeriod.Unset()
}

func (o BudgetStore) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BudgetStore) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	if !IsNil(o.Active) {
		toSerialize["active"] = o.Active
	}
	// skip: order is readOnly
	if o.Notes.IsSet() {
		toSerialize["notes"] = o.Notes.Get()
	}
	if o.AutoBudgetType.IsSet() {
		toSerialize["auto_budget_type"] = o.AutoBudgetType.Get()
	}
	if o.AutoBudgetCurrencyId.IsSet() {
		toSerialize["auto_budget_currency_id"] = o.AutoBudgetCurrencyId.Get()
	}
	if o.AutoBudgetCurrencyCode.IsSet() {
		toSerialize["auto_budget_currency_code"] = o.AutoBudgetCurrencyCode.Get()
	}
	if o.AutoBudgetAmount.IsSet() {
		toSerialize["auto_budget_amount"] = o.AutoBudgetAmount.Get()
	}
	if o.AutoBudgetPeriod.IsSet() {
		toSerialize["auto_budget_period"] = o.AutoBudgetPeriod.Get()
	}
	return toSerialize, nil
}

type NullableBudgetStore struct {
	value *BudgetStore
	isSet bool
}

func (v NullableBudgetStore) Get() *BudgetStore {
	return v.value
}

func (v *NullableBudgetStore) Set(val *BudgetStore) {
	v.value = val
	v.isSet = true
}

func (v NullableBudgetStore) IsSet() bool {
	return v.isSet
}

func (v *NullableBudgetStore) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBudgetStore(val *BudgetStore) *NullableBudgetStore {
	return &NullableBudgetStore{value: val, isSet: true}
}

func (v NullableBudgetStore) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBudgetStore) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


